...
fig = px.scatter_mapbox(
    ...
    title="Crime Hotspot Overview — San Diego, Los Angeles, San Francisco"
)
...
## California Crime Overview Dashboard
...
print(f"✓ San Diego: {len(df_sd):,} crimes")
print(f"✓ Los Angeles: {len(df_la):,} crimes")
print(f"✓ San Francisco: {len(df_sf):,} crimes")
...
try:
    ...
    df_sd["city"] = "San Diego"
    dataframes.append(df_sd)
    print(f"✓ Loaded San Diego: {len(df_sd)} rows")
except Exception as e:
    print(f"✗ Could not load San Diego: {e}")
...
try:
    ...
    df_la["city"] = "Los Angeles"
    dataframes.append(df_la)
    print(f"✓ Loaded Los Angeles: {len(df_la)} rows")
except Exception as e:
    print(f"✗ Could not load Los Angeles: {e}")
...
try:
    ...
    df_sf["city"] = "San Francisco"
    dataframes.append(df_sf)
    print(f"✓ Loaded San Francisco: {len(df_sf)} rows")
except Exception as e:
    print(f"✗ Could not load San Francisco: {e}")
...
# Remove invalid coordinates (0,0 or extreme outliers)
df_map = df_map[
    (df_map['latitude'] != 0) &
    (df_map['longitude'] != 0) &
    (df_map['latitude'].between(32, 38)) &  # California latitude range
    (df_map['longitude'].between(-125, -114))  # California longitude range
]

print(f"Valid coordinates: {len(df_map)} rows ({len(df_map)/len(df)*100:.1f}%)")

cities = df_map['city'].unique().tolist()
colors = ["#2E86AB", "#A23B72", "#F18F01"]

print(f"\nCrime counts by city:")
for city in cities:
    city_count = len(df_map[df_map['city'] == city])
    print(f"  {city}: {city_count:,} incidents with valid coordinates")
...
# Add density heatmap for each city
for i, city in enumerate(cities):
    city_data = df_map[df_map['city'] == city]

    # Define color scales for each city
    color_scales = [
        [[0, 'rgba(0,0,0,0)'], [0.3, 'rgba(46,134,171,0.25)'], [0.6, 'rgba(46,134,171,0.5)'], [1, 'rgba(46,134,171,0.8)']],  # Blue
        [[0, 'rgba(0,0,0,0)'], [0.3, 'rgba(162,59,114,0.25)'], [0.6, 'rgba(162,59,114,0.5)'], [1, 'rgba(162,59,114,0.8)']],  # Magenta
        [[0, 'rgba(0,0,0,0)'], [0.3, 'rgba(241,143,1,0.25)'], [0.6, 'rgba(241,143,1,0.5)'], [1, 'rgba(241,143,1,0.8)']]      # Orange
    ]

    fig.add_trace(go.Densitymapbox(
        lat=city_data['latitude'],
        lon=city_data['longitude'],
        z=[1] * len(city_data),  # All points weighted equally
        radius=8,
        colorscale=color_scales[i % len(color_scales)],
        showscale=False,
        hovertemplate=f'<b>{city}</b><br>Crime Density<extra></extra>',
        visible=(i == 0),
        name=city
    ))
...
for i, city in enumerate(cities):
    ...
    buttons.append(
        dict(
            label=f"{city} ({center['count']:,} incidents)",
            method="update",
            args=[
                {"visible": vis},
                {
                    ...
                    "mapbox.zoom": 10,
                    "title": {
                        "text": f"<b>Crime Heat Map — {city}</b><br><sub>{center['count']:,} incidents with valid coordinates</sub>"
                    }
                }
            ]
        )
...
fig.update_layout(
    ...
    title={
        "text": f"<b>Crime Heat Map — {first_city}</b><br><sub>{first_center['count']:,} incidents with valid coordinates</sub>",
        "x": 0.5,
        "xanchor": "center",
        "font": {"size": 20}
    },
    mapbox=dict(
        style="carto-positron",  # Clean base map
